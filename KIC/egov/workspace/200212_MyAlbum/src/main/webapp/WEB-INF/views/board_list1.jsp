...
<%@page import="com.album.model.AlbumBoardListTO"%>
...
<%
	...
	Object loginObj = session.getAttribute("album_isLogin");
	boolean isLogin = loginObj != null ? (boolean) loginObj : false;
	pageContext.include("url_pathfinder.jsp");
	
	/* 
	if(isLogin){		
		System.out.printf(" 로그인 상태!! id : %s, pw : %s", (String) session.getAttribute("album_id"), (String) session.getAttribute("album_pw"));		
	} else{
		System.out.println(" 로그아웃 상태!!");
	}
 */	

	AlbumBoardListTO listTO = (AlbumBoardListTO) request.getAttribute("listTO");
	...
	for (int i = 0; i < recordPerPage; ++i) {
		...
		sbHtml.append("						<a href='./view.do?cpage=" + cpage + "&seq=" + seq
				+ "'><img src='./upload/" + imgfilename + "' border='0' height='144px' /></a>");
		sbHtml.append("					</div>					");
		sbHtml.append("				</td>                       ");
		sbHtml.append("			</tr>                           ");
		sbHtml.append("			<tr>                            ");
					...
					<%
						...
						*/
						...
						if (startBlockPage == 1) {
							...
							out.println("<a href='./list.do?cpage=" + (startBlockPage - blockPerPage) + "'>&lt;&lt;</a>");
						}
						...
						if (cpage == 1) {
							...
							out.println("<span class='off'><a href='./list.do?cpage=" + (cpage - 1) + "'>&lt;</a>");
						}
						out.println("&nbsp;&nbsp;</span>");
						...
						if (cpage == totalPage) {
							...
							out.println("<a href='./list.do?cpage=" + (cpage + 1) + "'>&gt;</a>");
						}
