...
public class Day10_Java{
    ...
    public static void main(String[] args) 
    {
        ...
        if( argsList.contains("MainMethodMain") || argsList.size() <= 0  )
        {
            System.out.printf("\n\n\n");
        ...
        if( argsList.contains("Constructor1") || argsList.size() <= 0  )
        {
            System.out.printf("\n\n\n");
        ...
        if( argsList.contains("ThisMain") || argsList.size() <= 0 )
        {
            System.out.printf("\n\n\n");
        ...
        if( argsList.contains("StudentMain") || argsList.size() <= 0  )
        {
            System.out.printf("\n\n\n");
        ...
        if( argsList.contains("CodeBlockMain") || argsList.size() <= 0  )
        {
            System.out.printf("\n\n\n");

//            CodeBlock preCb = new CodeBlock();
            
            CodeBlock.y = 2;
            
            CodeBlock.staticMethod();
            System.out.println("more call staticM");
            CodeBlock.staticMethod();

            System.out.println();

            CodeBlock cb = new CodeBlock();
            System.out.println("new cb2");
        ...
        if( argsList.contains("Inheritance") || argsList.size() <= 0  )
        {
            System.out.printf("\n\n\n");
        ...
        if( argsList.contains("Inheritance2") || argsList.size() <= 0  )
        {
            System.out.printf("\n\n\n");
        ...
        if( argsList.contains("SuperMain") || argsList.size() <= 0  )
        {
            System.out.printf("\n\n\n");
        ...
        if( argsList.contains("InheritanceMain") || argsList.size() <= 0  )
        {
            System.out.printf("\n\n\n");
        ...
        if( argsList.contains("ModifierMain") || argsList.size() <= 0  )
        {
            System.out.printf("\n\n\n");
        ...
        if( argsList.contains("ModifierMain02") || argsList.size() <= 0  )
        {
            System.out.printf("\n\n\n");
...
class CodeBlock
{
    ...
    static //클래스 초기화 블록
    {
        System.out.println("1순위 static block 스태틱으로 호출하건 생성하건 가장먼저 호출된다");
    }

    {//인스턴스 초기화 블록
        System.out.println("2순위 instance block");
    }
    
    CodeBlock(){
        System.out.println("3순위 initalizer");        
    }
...
class Modifier{
    String dataDafault = "default Variable";
    public String dataPublic = "public Variable";
    protected String dataProtected = "protected Variable";
    private String dataPrivate = "private Variable";
