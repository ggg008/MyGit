...
//			sql = String.format("SELECT empno, ename, sal, sal*12+IFNULL(comm, 0) annual, hiredate FROM emp");
			sql = "SELECT empno, ename, sal, sal*12+IFNULL(comm, 0) annual, date_format(hiredate, '%y-%m-%d') hiredate FROM emp";//String.format 금칙 문자가 있음을 유의			
...
//				System.out.printf("%s %s %s\n", rs.getString(1), rs.getString(2), rs.getString(3));//숫자(컬럼순서)로도 가능
				...
				System.out.printf("%5s\t%5s\t%5s\t%10s\t%5s\n", 
						rs.getString(1), rs.getString(2), rs.getString(3), rs.getString("annual"), rs.getString("hiredate"));//별칭(alias)으로도 가져올수 있다
