...
public class JuminCheckerJOP
{
	public static String checkJumin(char[] cJu, Component comp) 
	{
		String errTitle = "에러입니당";
		String errMsg = "민증번호검사에 적합한 문자열이 아님!";
		if ((cJu.length <= 0 || 13 < cJu.length)){
			JOptionPane.showMessageDialog(comp, errMsg, errTitle, JOptionPane.ERROR_MESSAGE);
			return "";
		}
		...
		for (int iArg = 0; iArg < cJu.length; ++iArg) {
			...
			if (asciiNum < 48 || 57 < asciiNum) {
				...
				errMsg = "숫자가 아님  iArg : " + iArg + ", asciiNum : " + asciiNum;
				JOptionPane.showMessageDialog(comp, errMsg, errTitle, JOptionPane.ERROR_MESSAGE);
				return "";
			}
