...
<%
	...
	try {
		...
		String sql = "UPDATE rep_board1 SET hit=hit+1 WHERE seq = ?";//가능하면 *쓰지말고 다쓸것 
		...
		sql = "SELECT subject, writer, mail, wip, date_format(wdate, '%Y-%m-%d %h:%i:%s') fdate, hit, content FROM rep_board1 WHERE seq = ?";//가능하면 *쓰지말고 다쓸것 
		pstmt = conn.prepareStatement(sql);
		pstmt.setString(1, seq);
		...
		<div class="contents_sub">
			...
			<div class="btn_area">
				...
				<div class="align_right">
					...
					<button type="button" class="btn_write btn_txt01"
						style="cursor: pointer;"
						onclick="location.href='board_reply1.jsp?cpage=<%= cpage %>&seq=<%=seq%>'">답변</button>
				</div>
			</div>
			<!--//게시판-->
		</div>
