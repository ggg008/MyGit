		...
		$.ajax({
			...
			success : function(data) {				
				...
				$.each(data, function(i, element) {
					...
					html += "<td><button index="+ i +">수정</button></td>";
					html += "<td><button index="+ i +">삭제</button></td>";
					html += "</tr>";
					
					$("#users tbody" ).append(html);					
					$("#users tbody td button" ).button().on('click', function() {
						
						//console.log($(this).text() + '/' + $(this).attr('index'));
						//alert(data[i].name);
						
						console.log($("#users tbody tr" ).eq($(this).attr('index')).find('td').eq(0).text());
					});
